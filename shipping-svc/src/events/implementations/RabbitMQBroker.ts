...
export class RabbitMQBroker implements IMessageBroker, OnModuleInit {
    ...
    private readonly exchange = 'shipping_events_exchange';
    ...
    async publish(queue: string, message: any): Promise<void> {
        ...
        if (!this.channel) {
            ...
            console.log("[RabbitMQ] Connected");
            await publishChannel.assertExchange(this.exchange, 'fanout', { durable: true });
            publishChannel.publish(this.exchange, '', Buffer.from(JSON.stringify(message)));
